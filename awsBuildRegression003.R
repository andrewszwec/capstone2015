...
justRestAndCafe <- read.csv("justRestAndCafe.csv", stringsAsFactors=FALSE)

# get reviews
reviews <- justRestAndCafe[justRestAndCafe$review_id %in% raw_train$review_id, c("review_id", "text")]

## NON PARALLEL
# for each review in matrix split into letters and count number of special characters
for (i in 1:nrow(raw_train)) {
      b <- strsplit(raw_train$text[[i]], "")[[1]]        # Split text into letters
      raw_train$numSpecialChars[i] <- length(grep('[^\\w\\s]', b, ignore.case=TRUE, value=TRUE, perl=TRUE))  # count number of special chars
      raw_train$numCapitals[i] <- length(grep('[A-Z]', b, ignore.case=FALSE, value=TRUE, perl=TRUE))      
}
